...
# test 2 : reading a matrix in triplet form, writing it in CSC
...
if diff -w $OUT $EXPECTED >/dev/null ; then
  echo 'ok 2 - reading a matrix, converting it to CSC and writing it'
else
  echo 'not ok 2 -  reading a matrix, converting it to CSC and writing it'
fi
