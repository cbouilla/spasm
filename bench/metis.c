...
int main() {
  spasm_triplet *T;
  spasm *A, *B;
  int n, m, i, j, result, *Ap, *Aj, k, npart, edgecut, *row_part, *col_part,
     *p, *q, *qinv, px;
  ...
  /* in METIS mesh/hypergraph words : "element" = row and "node" = column */
  n = A->n;
  m = A->m;
  Ap = A->p;
  Aj = A->j;
  ...
  for (i = 0; i < n; i++) {
    for (px = Ap[i]; px < Ap[i + 1]; px++) {
      if (row_part[i] != col_part[Aj[px]]) {
        row_part[i] = npart;    /* row i belongs to separator */
        k++;
        break;
      }
    }
