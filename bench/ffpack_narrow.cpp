...
/** Finds cheap pivots, computes random dense linear combination of rows of the Schur complement, 
    then finish with a dense rank computation */
  ...
  #pragma omp parallel
  {
    ...
    for(int k=0; k<schur_n; k++) {
      ...
      for(int i = n_cheap; i < n; i++) {
        int inew = p[i];
        spasm_scatter(Aj, Ax, Ap[inew], Ap[inew + 1], rand_r(&seed) % prime, y, prime);
      }
