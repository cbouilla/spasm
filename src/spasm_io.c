...
spasm *spasm_load_gbla_old(FILE * f, int with_values) {
  ...
  /*
   * dirty hack : we load the 16-bit coefficient in the area dedicated to
   * storing the column indices (unused at this point). This avoids
...
spasm *spasm_load_gbla_new(FILE * f) {
  ...
  /*
   * read (and ignore) the polmaps. Dirty hack: we send them to the area for
   * the column indices
   */
  r = n;
